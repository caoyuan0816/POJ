...
void pushDown(int a,int b, int k, int l, int r){
	...
	if(b <= mid){
		pushDown(a, b, lch,  l,  mid);
	}else if(a >= mid+1){
		pushDown(a, b, rch, mid+1, r);
	}else{
		pushDown(a,   mid, lch,  l,  mid);
		pushDown(mid+1, b, rch, mid+1, r);
	}
